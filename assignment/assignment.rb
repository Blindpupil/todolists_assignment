...
class Assignment
  ...
  def find_allusers(offset, limit)
    ...
    # use User Model class to find all Users, ordered by `updated_at` ascending,
  ...
  def update_password(id, password_digest)
    ...
    # use the User Model class to update the `password_digest`
    # for the User associated with the id primary key
  ...
  def update_listname(id, name)
    ...
    # use the TodoList Model class to update the `list_name`
    # for the TodoList associated with id primary key
    # (no return is required)
